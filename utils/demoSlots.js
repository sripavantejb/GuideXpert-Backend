...
/**
 * When today and tomorrow are fully disabled, we must surface **two distinct IST
 * calendar days** that each have at least one bookable slot (after the 1h cutoff),
 * not merely two days that are enabled in admin but already hidden by time.
 */
async function getDemoSlotsTwoDateFallback(now) {
  ...
  /** @type {{ dateStr: string, slots: { id: string, label: string, date: string, enabled: boolean, selectionId: string }[] }[]} */
  const daysWithVisibleSlots = [];

  for (let i = 0; i < MAX_FALLBACK_SCAN_DAYS && daysWithVisibleSlots.length < 2; i++) {
