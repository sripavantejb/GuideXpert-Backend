...
async function tryLlmReply({
  ...
  const llm = await answerWithTimeout({
    inboundText,
    conversationId,
    leadContext,
    languageMetadata,
  ...
  if (llm && llm.text) {
    return {
      text: String(llm.text).trim().slice(0, 3500),
      model: llm.model,
      guardrailModified: llm.guardrailModified,
      guardrailReason: llm.guardrailReason,
      languageLog: llm.languageLog,
    };
  }
