  ...
  try {
    ...
    if (uniqueByMobile) {
      ...
      const [records, totalRecords, uniqueAgg] = await Promise.all([
        TrainingAttendance.aggregate(pipeline),
        TrainingAttendance.countDocuments(match),
        TrainingAttendance.aggregate(statsPipeline)
      ]);
    ...
    console.error('[getTrainingAttendance] Error:', error);
