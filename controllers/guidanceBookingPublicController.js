...
const { sendGuidanceCounsellorBookingNotifyForBooking } = require('../services/guidanceCounsellorNotifyService');
const {
...
exports.bookSlot = async (req, res) => {
  try {
    ...
    try {
      whatsappCounsellor = await sendGuidanceCounsellorBookingNotifyForBooking(
        result.lead,
        result.slot,
        result.counselor
      );
      if (whatsappCounsellor?.attempted && !whatsappCounsellor?.success) {
        console.warn(
          '[bookSlot] WhatsApp guidance_counsellor_booking_notify unsuccessful:',
          whatsappCounsellor.error || whatsappCounsellor.skippedReason
        );
      }
    } catch (waErr) {
      const msg = String(waErr?.message || waErr || 'exception').slice(0, 240);
